// ============================================================
//  3D CURVED PROJECT CAROUSEL — slow wheel-driven transitions
// ============================================================
export async function initCarousel(canvasId, projects) {
  ...
  floor.rotation.x = -Math.PI / 2; floor.position.set(0, -2.2, -6.5); room.add(floor);
  const back = new THREE.Mesh(new THREE.PlaneGeometry(34, 18, 32, 20), wireMat);
  back.position.set(0, 3.8, -13); room.add(back);
  const left = new THREE.Mesh(new THREE.PlaneGeometry(18, 16, 18, 20), wireMat);
  left.position.set(-12, 3.5, -6.5); left.rotation.y = Math.PI / 2.15; room.add(left);
  const right = new THREE.Mesh(new THREE.PlaneGeometry(18, 16, 18, 20), wireMat);
  right.position.set(12, 3.5, -6.5); right.rotation.y = -Math.PI / 2.15; room.add(right);
  scene.add(room);

  const loader = new THREE.TextureLoader();
  const textures = await Promise.all(projects.map(p => new Promise(r => loader.load(p.image, r, undefined, () => r(null)))));
  ...
  function updateHUD(active) {
    ...
    document.getElementById('carousel-tags').innerHTML = p.tags.map(t => `<span class="ctag">${t}</span>`).join('');
    document.querySelectorAll('.cdot').forEach((d, i) => d.classList.toggle('active', i === active));
  }
  ...
  projects.forEach((_, i) => {
    ...
    d.addEventListener('click', () => { currentIdx = i; targetOffset = i; updateHUD(i); });
  ...
  function animate() {
    ...
    camera.position.y += ((-my * 0.18 + 0.42) - camera.position.y) * 0.04;
    camera.lookAt(0, 0.2, -6.9);
    room.rotation.y += 0.0007;

    cards.forEach((card, i) => {
      const rel = i - currentOffset;
      const a = rel * spacing;
      card.position.x = Math.sin(a) * R;
      card.position.z = Math.cos(a) * R - (R + 3.0);
      card.position.y = Math.sin(performance.now() * 0.001 + i * 0.65) * 0.04 + 0.15;
      card.rotation.y = -a * 0.98;
      const active = Math.abs(i - currentIdx) < 0.001;
      const targetScale = active ? 1.18 : 0.68;
      card.scale.lerp(new THREE.Vector3(targetScale, targetScale, 1), 0.06);
      card.material.opacity += ((active ? 1 : 0.24) - card.material.opacity) * 0.08;
    });
